...
describe('The mutateSource module', function () {
  ...
  /**
   * Returns an AST generated by falafel. falafel adds attributes such as
   * node.parent which many of these methods rely upon. However, falafel does
   * not return its AST, so it is taken from the reverse traversal where the
   * last node visited is the root.
   */
  function getFalafelAST(source) {
    var root;
    falafel(source, function (node) { root = node; });
    return root;
  }
